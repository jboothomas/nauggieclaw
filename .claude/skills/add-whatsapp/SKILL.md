...
4. Service is running: `launchctl list | grep nauggieclaw` (macOS) or `systemctl --user status nauggieclaw` (Linux)
5. Logs: `tail -50 logs/nauggieclaw.log`
...
4. Rebuild and restart: `npm run build && launchctl kickstart -k gui/$(id -u)/com.nauggieclaw` (macOS) or `npm run build && systemctl --user restart nauggieclaw` (Linux)
