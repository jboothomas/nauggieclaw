...
Write `/tmp/nauggieclaw-diagnostics.json`. No paths, usernames, hostnames, or IP addresses.
...
**Yes**:
```bash
curl -s -X POST https://us.i.posthog.com/capture/ -H 'Content-Type: application/json' -d @/tmp/nauggieclaw-diagnostics.json
rm /tmp/nauggieclaw-diagnostics.json
```

**No**: `rm /tmp/nauggieclaw-diagnostics.json`
...
4. `rm /tmp/nauggieclaw-diagnostics.json`
