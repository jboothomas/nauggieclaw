...
If fork: instruct the user to fork `jboothomas/nauggieclaw` on GitHub (they need to do this in their browser), then ask them for their GitHub username. Run:
```bash
git remote rename origin upstream
git remote add origin https://github.com/<their-username>/nauggieclaw.git
...
**Case C — both `origin` (user's fork) and `upstream` (jboothomas) exist:**

Already configured. Continue.

**Verify:** `git remote -v` should show `origin` → user's repo, `upstream` → `jboothomas/nauggieclaw.git`.
...
**If the command succeeds** (outputs a SESSION= line): authentication is already configured. Continue to step 5.

**If `auggie` is not found:** Auggie is part of Augment Code. Tell the user to install it from [augmentcode.com](https://augmentcode.com) and then return here.

**If `auggie token print` fails or shows no token:** the user needs to log in:
```bash
auggie login
```

This opens a browser for authentication. Wait for the user to confirm login completed, then re-run `auggie token print` to verify.

**Optional — pin the token to `.env`** (useful on servers or if you want a stable token that doesn't require `auggie login` on each reboot):
```bash
...
- macOS: `launchctl unload ~/Library/LaunchAgents/com.nauggieclaw.plist`
- Linux: `systemctl --user stop nauggieclaw` (or `systemctl stop nauggieclaw` if root)

Run `npx tsx setup/index.ts --step service` and parse the status block.

**If FALLBACK=wsl_no_systemd:** WSL without systemd detected. Tell user they can either enable systemd in WSL (`echo -e "[boot]\nsystemd=true" | sudo tee /etc/wsl.conf` then restart WSL) or use the generated `start-nauggieclaw.sh` wrapper.
...
- macOS: check `launchctl list | grep nauggieclaw`. If PID=`-` and status non-zero, read `logs/nauggieclaw.error.log`.
- Linux: check `systemctl --user status nauggieclaw`.
...
- SERVICE=stopped → `npm run build`, then restart: `launchctl kickstart -k gui/$(id -u)/com.nauggieclaw` (macOS) or `systemctl --user restart nauggieclaw` (Linux) or `bash start-nauggieclaw.sh` (WSL nohup)
- SERVICE=not_found → re-run step 7
- CREDENTIALS=missing → re-run step 4: run `auggie login` then verify with `auggie token print`
- CHANNEL_AUTH shows `not_found` for any channel → re-invoke that channel's skill (e.g. `/add-telegram`)
- REGISTERED_GROUPS=0 → re-invoke the channel skills from step 5
- MOUNT_ALLOWLIST=missing → `npx tsx setup/index.ts --step mounts -- --empty`

Tell user to test: send a message in their registered chat. Show: `tail -f logs/nauggieclaw.log`

## Troubleshooting

**Service not starting:** Check `logs/nauggieclaw.error.log`. Common: wrong Node path (re-run step 7), credential system not running (Docker: check `curl ${ONECLI_URL}/api/health`; Apple Container: check `.env` credentials), missing channel credentials (re-invoke channel skill).

**Container agent fails ("Claude Code process exited with code 1"):** Ensure the container runtime is running — `open -a Docker` (macOS Docker), `container system start` (Apple Container), or `sudo systemctl start docker` (Linux). Check container logs in `groups/main/logs/container-*.log`.

**No response to messages:** Check trigger pattern. Main channel doesn't need prefix. Check DB: `npx tsx setup/index.ts --step verify`. Check `logs/nauggieclaw.log`.

**Channel not connecting:** Verify the channel's credentials are set in `.env`. Channels auto-enable when their credentials are present. For WhatsApp: check `store/auth/creds.json` exists. For token-based channels: check token values in `.env`. Restart the service after any `.env` change.

**Unload service:** macOS: `launchctl unload ~/Library/LaunchAgents/com.nauggieclaw.plist` | Linux: `systemctl --user stop nauggieclaw`
