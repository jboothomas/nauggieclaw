...
Tell the user to send themselves a test email. The agent should pick it up within a minute. Monitor: `tail -f logs/nauggieclaw.log | grep -iE "(gmail|email)"`.
...
5. Rebuild: `cd container && ./build.sh && cd .. && npm run build && launchctl kickstart -k gui/$(id -u)/com.nauggieclaw` (macOS) or `systemctl --user restart nauggieclaw` (Linux)
...
8. Rebuild: `cd container && ./build.sh && cd .. && npm run build && launchctl kickstart -k gui/$(id -u)/com.nauggieclaw` (macOS) or `systemctl --user restart nauggieclaw` (Linux)
