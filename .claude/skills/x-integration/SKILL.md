---
name: x-integration
description: X (Twitter) integration for NauggieClaww. Post tweets, like, reply, retweet, and quote. Use for setup, testing, or troubleshooting X functionality. Triggers on "setup x", "x integration", "twitter", "post tweet", "tweet".
---
...
> **Compatibility:** NauggieClaww v1.0.0. Directory structure may change in future versions.
...
1. **NauggieClaww is installed and running** - WhatsApp connected, service active
...
# Verify: launchctl list | grep nauggieclaw (macOS) or systemctl --user status nauggieclaw (Linux)
```
...
| `NAUGGIECLAW_ROOT` | `process.cwd()` | Project root directory |
...
| `logs/nauggieclaw.log` | Service logs (contains X operation logs) | Ignored |
...
To integrate this skill into NauggieClaww, make the following modifications:
...
All paths below are relative to project root (`NAUGGIECLAW_ROOT`).
...
**Verify success:**
```bash
launchctl list | grep nauggieclaw  # macOS — should show PID and exit code 0 or -
# Linux: systemctl --user status nauggieclaw
```
...
grep -i "x_post\|x_like\|x_reply\|handleXIpc" logs/nauggieclaw.log | tail -20

# Script errors
grep -i "error\|failed" logs/nauggieclaw.log | tail -20
```
...
docker run nauggieclaw-agent ls -la /app/src/skills/
```
