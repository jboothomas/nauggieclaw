...
```bash
grep -q 'nauggieclaw proxy' /etc/pf.conf 2>/dev/null || echo '# nauggieclaw proxy — block LAN access to credential proxy
...
```bash
echo '{}' | container run -i --entrypoint /bin/echo nauggieclaw-agent:latest "Container OK"
```
