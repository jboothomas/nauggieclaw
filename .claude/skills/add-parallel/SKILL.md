...
Add Parallel AI MCP servers after the nauggieclaw server:
```typescript
const mcpServers: Record<string, any> = {
  nauggieclaw: ipcMcp
};
...
3. Create a polling scheduled task using `mcp__nauggieclaw__schedule_task`:
   ```
   ...
   3. Send results to user with mcp__nauggieclaw__send_message
   4. Use mcp__nauggieclaw__complete_scheduled_task to mark this task as done
...
Verify the build:
```bash
echo '{}' | docker run -i --entrypoint /bin/echo nauggieclaw-agent:latest "Container OK"
```
...
```bash
sleep 3
launchctl list | grep nauggieclaw  # macOS
...
- Check task runs: `tail -f logs/nauggieclaw.log | grep "scheduled task"`
...
5. Restart: `launchctl kickstart -k gui/$(id -u)/com.nauggieclaw` (macOS) or `systemctl --user restart nauggieclaw` (Linux)
