...
class StatusBarController: NSObject {
    ...
    private func checkRunning() -> Bool {
        ...
        task.arguments = ["list", "com.nauggieclaw"]
        let pipe = Pipe()
    ...
    @objc private func restartService() {
        let uid = getuid()
        run("/bin/launchctl", ["kickstart", "-k", "gui/\(uid)/com.nauggieclaw"])
        refresh(after: 3)
    }

    @objc private func viewLogs() {
        let logPath = "\(StatusBarController.projectRoot)/logs/nauggieclaw.log"
