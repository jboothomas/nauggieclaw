...
claw --image nauggieclaw-agent:dev "Hello"
...
The default timeout is 300 seconds. For longer tasks, pass `--timeout 600` (or higher). If the container consistently hangs, check that your `nauggieclaw-agent:latest` image is up to date by running `./container/build.sh`.
...
```bash
export NAUGGIECLAW_DIR=/path/to/your/nauggieclaw
```
